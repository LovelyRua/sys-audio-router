...
WindowsVirtualAsioTransportSession::stats() const noexcept {
  return {
      ...
      capture_bus_underflows_.load(std::memory_order_relaxed),
...
WindowsVirtualAsioTransportSession::WindowsVirtualAsioTransportSession(
    ...
    : mapping_(std::move(mapping)),
      ...
      capture_consumer_(std::move(capture_consumer)),
...
void WindowsVirtualAsioTransportSession::process_available_input() noexcept {
  ...
  for (std::size_t block = 0; block < kMaximumBlocksPerWake; ++block) {
    ...
    if (capture_consumer_.valid()) {
      output_buffer_.clear();
      if (!capture_consumer_.read(output_buffer_)) {
        capture_bus_underflows_.fetch_add(1, std::memory_order_relaxed);
      }
    } else {
