...
int main(int argc, char** argv) {
  ...
  if (!sar::tools::parse_wasapi_measure_options(argc, argv, options)) return 2;
  ...
  sar::tools::print_wasapi_probe(std::cout, "Default loopback stream", probe);
  ...
  auto open_result = sar::platform::open_default_wasapi_loopback_loop(graph, diagnostics);
  if (!open_result.ok()) {
  ...
  if (!errors.empty()) {
    ...
    for (const auto& error : errors) std::cerr << error.code << ": " << error.message << '\n';
  }
  const auto summary = loop->summary();
  sar::tools::print_wasapi_runtime_summary(std::cout, summary.runtime);
  sar::tools::print_wasapi_stream_diagnostics(std::cout, "Loopback stream diagnostics",
                                              summary.capture_stream);
  ...
  if (sar::platform::wasapi_runtime_summary_should_fail(summary.runtime,
                                                        options.require_healthy)) return 1;
