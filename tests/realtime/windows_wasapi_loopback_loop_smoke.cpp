...
int main() {
  if (!has_default_output_device()) {
    std::cout << "Windows WASAPI loopback loop skipped: "
                 "no default output endpoint\n";
    return 0;
  }
  ...
  sar::graph::Graph graph(1,
                          probe.probe().mix_format.channels,
                          probe.probe().buffer_frames,
  ...
  if (!open.ok()) {
    for (const auto& error : open.errors()) {
  ...
  if (const auto failure =
          expect(loop->capture_diagnostics().mode ==
                     sar::platform::WasapiStreamMode::Loopback,
                 "Expected loopback capture mode")) {
    return failure;
  }

  const auto start = loop->start(10);
  if (!start.ok()) {
    for (const auto& error : start.errors()) {
      std::cerr << error.code << ": " << error.message << '\n';
    }
    return 1;
  }
  ...
  if (const auto failure = expect(loop->read_clock(clock_snapshot),
                                  "Expected loopback clock snapshot")) {
    return failure;
  }
  if (const auto failure = expect(clock_snapshot.frequency > 0,
                                  "Expected loopback clock frequency")) {
    return failure;
  }
  ...
  if (const auto failure =
          expect(!summary.running, "Expected stopped loopback loop")) {
    return failure;
  }
  if (const auto failure =
          expect(summary.capture_stream.mode ==
                     sar::platform::WasapiStreamMode::Loopback,
                 "Expected loopback summary mode")) {
    return failure;
  }
  if (const auto failure =
          expect(summary.runtime.has_capture_stream &&
                     !summary.runtime.has_render_stream,
                 "Expected capture-only loopback runtime summary")) {
    return failure;
  }
  if (const auto failure =
          expect(summary.runtime.health !=
                     sar::platform::WasapiRuntimeHealth::Faulted,
                 "Expected loopback runtime without fault")) {
    return failure;
  }
